...
use crate::utils::{sample, types::Element};
...
impl Proof {
    ...
    /// Calls up to setup.max_retries times the prove_index function and
    /// returns a `Proof` if a suitable candidate tuple is found.
    pub(super) fn new(setup: &Setup, prover_set: &[Element]) -> Option<Self> {
        // Run prove_index up to max_retries times
        (0..setup.max_retries).find_map(|retry_counter| {
            Self::prove_index(setup.set_size, setup, prover_set, retry_counter).1
        })
    }
    ...
    pub(super) fn verify(&self, setup: &Setup) -> bool {
        if self.search_counter >= setup.search_width
            || self.retry_counter >= setup.max_retries
            || self.element_sequence.len() as u64 != setup.proof_size
        {
            return false;
        }

        // Initialise a round with given retry and search counters
        let Some(mut round) = Round::new(self.retry_counter, self.search_counter, setup.set_size)
        else {
            return false;
        };
        ...
        for &element in &self.element_sequence {
            ...
            let Some(bin_id) = Proof::bin_hash(setup.set_size, self.retry_counter, element) else {
                return false;
            };
        ...
        Proof::proof_hash(setup.valid_proof_probability, &round)
    }

    /// Indexed proving algorithm, returns the total number of DFS calls done
    /// to find a proof and Some(proof) if found within setup.dfs_bound calls
    ...
    fn prove_index(
        ...
        // Run the DFS algorithm on up to setup.search_width different trees
        let mut step = 0;
        for search_counter in 0..setup.search_width {
            ...
            if let Some(r) = Round::new(retry_counter, search_counter, set_size) {
                ...
                let (dfs_calls, proof_opt) = Self::dfs(setup, &bins, &r, step.saturating_add(1));
    ...
    /// done and, if not found under setup.dfs_bound calls, None otherwise
    ...
    fn dfs(
        ...
        // If current round comprises setup.proof_size elements and satisfies
        // the proof_hash check, return it cast as a Proof
        if round.element_sequence.len() as u64 == setup.proof_size {
            let proof_opt = if Proof::proof_hash(setup.valid_proof_probability, round) {
                Some(Self {
        ...
        for &element in &bins[round.id as usize] {
            // If DFS was called more than setup.dfs_bound times, abort this
            // round
            if step == setup.dfs_bound {
                return (step, None);
            }
