...
/// Return N-byte hash of the given list of data
pub fn hash<const N: usize>(hash_list: Vec<Vec<u8>>) -> [u8; N] {
...
pub fn gen_weighted_items<const N: usize>(
    ...
    for b in 0..set_size {
        ...
        data.push(b.to_ne_bytes().to_vec());
        let item = hash::<N>(data);
        let weight = u32::from_be_bytes(hash::<4>(vec![item.to_vec()]));
        sum += weight as u64;
        s_p.push(item);
        s_n.push(weight);
    }
